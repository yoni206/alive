...
(declare-fun %Z() Int)
(assert (in_range k %Z))
(assert (everything_is_ok_for k %Z))


(declare-fun %RHS() Int)
(assert (in_range k %RHS))
(assert (everything_is_ok_for k %RHS))
...
(assert (and (= C2 (intnot k C1)) (not (= (intadd k (intadd k (intxor k (intor k %Z C2) C1) 1) %RHS) (intsub k %RHS (intand k %Z C1))))))
(assert true)
(check-sat)