...
(declare-fun %d() Int)
(assert (in_range k %d))
(assert (everything_is_ok_for k %d))
...
(assert
(let ((?x18568 (ite (= (intand k %a %d) %d) 1 0)))
(let ((?x13029 (ite (= (intand k %a %b) %b) 1 0)))
(and (distinct (intand k ?x13029 ?x18568) (ite (= (intand k %a (intor k %b %d)) (intor k %b %d)) 1 0)) true))))
(check-sat)