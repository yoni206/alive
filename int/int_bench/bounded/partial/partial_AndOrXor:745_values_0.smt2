...
(assert
(let ((?x9050 (ite (and (distinct (intand k %a C1) %a) true) 1 0)))
(let (($x7176 (and (distinct (intand k ?x9050 (ite (and (distinct (intand k %a C2) %a) true) 1 0)) ?x9050) true)))
(and (= (intor k C1 C2) C1) $x7176))))
(check-sat)