...
(assert
(let ((?x23527 (ite (< (intadd k %X (intneg k C1)) (intadd k (intneg k C1) C2)) 1 0)))
(let ((?x3703 (ite (intslt k %X C2) 1 0)))
(let ((?x17956 (ite (intsge k %X C1) 1 0)))
(and (intsle k C1 C2) (and (distinct (intand k ?x17956 ?x3703) ?x23527) true))))))
(check-sat)