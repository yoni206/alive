...
(declare-fun |ana_isPowerOf2(abs(C))|() Int)
(assert (in_range k |ana_isPowerOf2(abs(C))|))
(assert (everything_is_ok_for k |ana_isPowerOf2(abs(C))|))


(declare-fun termITE_1() Int)
(assert (in_range k termITE_1))
(assert (everything_is_ok_for k termITE_1))


(assert (and (intslt k C 0) (not (= (intadd k (intmul k C %Y) (intadd k (intmul k C C1) (intadd k (intmul k %Y termITE_1) (intmul k C1 termITE_1)))) 0)) (not (= termITE_1 0))))
(assert (and (intslt k C 0) (not (= (intadd k (intmul k C %Y) (intadd k (intmul k C C1) (intadd k (intmul k %Y termITE_1) (intmul k C1 termITE_1)))) 0)) (not (= termITE_1 0)) (= (intand k termITE_1 (intadd k termITE_1 (intmax k))) 0)))
(assert (ite (not (intslt k C 0)) (= termITE_1 C) (= termITE_1 (intneg k C))))
(check-sat)