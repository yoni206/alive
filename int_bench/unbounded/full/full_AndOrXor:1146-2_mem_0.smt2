...
(declare-fun mem0() Int)
(assert (in_range k mem0))
(assert (everything_is_ok_for k mem0))
...
(assert (let ((_let_0 (= |ana_MaskedValueIsZero(%op0RHS, ~C)| 1))) (and (=> _let_0 (= (intand k %op0RHS (intnot k C)) 0)) _let_0 (not (= mem0 mem0)))))
(assert true)
(check-sat)