...
(declare-fun |ana_MaskedValueIsZero(%op0LHS, ~C)|() Int)
(assert (in_range k |ana_MaskedValueIsZero(%op0LHS, ~C)|))
(assert (everything_is_ok_for k |ana_MaskedValueIsZero(%op0LHS, ~C)|))


(assert (not (= (intand k C (intor k %op0RHS %op0LHS)) (intor k %op0LHS (intand k C %op0RHS)))))
(assert (= 0 (intand k %op0LHS (intnot k C))))
(check-sat)