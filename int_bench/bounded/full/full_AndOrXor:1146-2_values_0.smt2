...
(declare-fun %op0RHS() Int)
(assert (in_range k %op0RHS))
(assert (everything_is_ok_for k %op0RHS))
...
(declare-fun %op0LHS() Int)
(assert (in_range k %op0LHS))
(assert (everything_is_ok_for k %op0LHS))


(declare-fun |ana_MaskedValueIsZero(%op0RHS, ~C)|() Int)
(assert (in_range k |ana_MaskedValueIsZero(%op0RHS, ~C)|))
(assert (everything_is_ok_for k |ana_MaskedValueIsZero(%op0RHS, ~C)|))


(assert (let ((_let_0 (= |ana_MaskedValueIsZero(%op0RHS, ~C)| 1))) (and (=> _let_0 (= (intand k %op0RHS (intnot k C)) 0)) _let_0 (not (= (intand k (intor k %op0LHS %op0RHS) C) (intor k (intand k %op0LHS C) %op0RHS))))))
(assert true)
(check-sat)