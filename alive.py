...
def check_refinement(srcv, tgtv, types, extra_cnstrs, users, opt_name, index):
  for k,v in srcv.iteritems():
    ...
    check_expr(qvars, base_cnstr + [mk_not(defb)], lambda s :
      ("Domain of definedness of Target is smaller than Source's for %s %s\n"
         % (var_type(k, types), k),
       str_model(s, a), 'undef', k, srcv, tgtv, types), opt_name, index, "undef")
    ...
    check_expr(qvars, base_cnstr + [mk_not(poisonb)], lambda s :
      ("Domain of poisoness of Target is smaller than Source's for %s %s\n"
         % (var_type(k, types), k),
       str_model(s, a), 'poison', k, srcv, tgtv, types), opt_name, index, "poison")

    # Check that final values of vars are equal.
    check_expr(qvars, base_cnstr + [a != b], lambda s :
      ("Mismatch in values of %s %s\n" % (var_type(k, types), k),
       str_model(s, a), str_model(s, b), k, srcv, tgtv, types), opt_name,index,  "values")
  ...
  for k,v in srcv.bb_pres.iteritems():
    ...
    check_expr([], [p1 != p2] + extra_cnstrs, lambda s :
      ("Mismatch in preconditions for BB '%s'\n" % k, str_model(s, p1),
       str_model(s, p2), None, srcv, tgtv, types), opt_name, index, "preBB")
...
#    i = i+1
    ...
    check_typed_opt(pre, src, ident_src, tgt, ident_tgt, types, users, name, proofs)
...
def is_good(indexes_to_types):
    result = True
    for index in indexes_to_types:
        types = indexes_to_types[index]
        if len(types) != 1:
            result = False
            break
    return result
...
def pretty_print(d, indent=0):
    for k in d:
        ...
        if isinstance(d[k] , dict):
            pretty_print(d[k], indent + 2)
        else:
            print_indents(indent+2)
            print d[k]

def print_indents(indent):
